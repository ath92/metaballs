...
const float hitThreshold = 0.0045;

const int CAMERA_ITERATIONS = 50;
const int LIGHT_ITERATIONS= 40;
...
float doModel(vec3 p) {
    ...
    for (float i = 1.; i <= 8.; i++) {
        d = smin(
                ...
                sphere(
                    p,
                    vec3(3. * sin(i * rx * t), 1.5 * sin(i * ry * t), 1. * sin(i * rz * t)),
                    0.2
                ),
                2.
            );
...
void main() {
    ...
    vec3 collision;
    float fog;
    float lightStrength = trace(cameraPosition - vec3(0,0,5), direction, collision, iterations, fog);
